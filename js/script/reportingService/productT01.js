...
var gedai999999Arr = ['A_REB_LOAN_SIZE_SITU_TAB-LS034','A_REB_LOAN_SIZE_SITU_TAB-LS035','A_REB_LOAN_SIZE_SITU_TAB-LS036'
                      ...
                      ,'A_PLF_CUST_LOAN_TAB-JC005','A_PLF_CUST_LOAN_TAB-JC006','A_PLF_CUST_AMT_FUND_TAB-JC007','A_REB_FINANC_STAT_TAB-LS048'
                      ,'A_COB_CNY_CORP_CAPT_QTTY_DTL-GS021'];
//排序管辖行-支行
...
//排序显示机构SHOW_ORG_NUM
var orderShowOrgNum = ['A_COB_IN_OFF_BAL_BIZ_SITU_TAB-GS015']
//默认分页最多50行
...
function createQueryParams(params, columns) {
                ...
                if (item.field != null && item.field != undefined) {
                	if(item.matrixingTag != undefined && item.matrixingTag != null){
                		...
                		if(item.field =='m_index_cmplt_ratio' && whereList[0].tableName=='A_REB_CAPT_REP_REG_LINE-LS016'){//LS016
                			...
                			res +=", (case when v.NEGU_INDEX_VAL-A.VIP_CUST_CNT_YB_INCREMT<0 then 0 else v.NEGU_INDEX_VAL-A.VIP_CUST_CNT_YB_INCREMT end) as "+item.field;
                		}else if(item.field =='m_index_cmplt_ratio' && whereList[0].tableName=='A_REB_VIP_INDEX_REP_ORG-LS025'){//LS025
                			res +=", v.NEGU_INDEX_CMPLT_RATIO  as "+item.field;
                		}else if(item.field =='yb_avg_brch_inc' && whereList[0].tableName=='A_REB_VIP_INDEX_REP_ORG-LS025'){//LS025
                			res +=", round(A."+item.field +"#2) as "+item.field;
                			groupBy.push("A."+item.field);
                		}else{
                			//字段脱敏
    ...
    queryParams.tableStr = function () {
       // return whereList[0].dataBaseUser+'.'+whereList[0].tableName.split('-')[0];
    	...
    	if (currCdVal == '01') {//人民币
    		if(whereList[0].tableName=="A_REB_CAPT_REP_REG_LINE-LS016"){//LS016
    			res += " and V.INDEX_NUM='KRE0052' ";
    		}else if(whereList[0].tableName=="A_REB_DPST_REP_REG_LINE-LS018"){//LS018
    			res += " and V.INDEX_NUM='KRE0057' ";
    		}else if(whereList[0].tableName=="A_REB_LOAN_SIZE_SITU_TAB-LS034"){//LS034
    			res += " and V.INDEX_NUM='KRE0068' ";
    		}
        } else if (currCdVal == '02') {//外折人
        	if(whereList[0].tableName=="A_REB_CAPT_REP_REG_LINE-LS016"){//LS016
        		res += " and V.INDEX_NUM='KRE0053' ";
    		}else if(whereList[0].tableName=="A_REB_DPST_REP_REG_LINE-LS018"){//LS018
    			res += " and V.INDEX_NUM='KRE0058' ";
    		}else if(whereList[0].tableName=="A_REB_LOAN_SIZE_SITU_TAB-LS034"){//LS034
    			res += " and V.INDEX_NUM='KRE0069' ";
    		}
        } else{//本外币
        	if(whereList[0].tableName=="A_REB_CAPT_REP_REG_LINE-LS016"){//LS016
        		res += " and V.INDEX_NUM='KRE0001' ";
    		}else if(whereList[0].tableName=="A_REB_DPST_REP_REG_LINE-LS018"){//LS018
    			res += " and V.INDEX_NUM='KRE0002' ";
    		}else if(whereList[0].tableName=="A_REB_LOAN_SIZE_SITU_TAB-LS034"){//LS034
    			res += " and V.INDEX_NUM='KRE0003' ";
    		}
        }
    	if(whereList[0].tableName=="A_REB_VIP_INDEX_REP_ORG-LS025"){
    		res += " and V.INDEX_NUM='KRE0146' ";
    	}
        return res;
    ...
    queryParams.orderStr = function () {
        ...
        if(orderShowOrgNum.indexOf(whereList[0].tableName)>-1){
        	res += ',A.show_org_num '
        }


        if(whereList[0].tableName == 'A_REB_PAY_AGEN_CUST_PROD_SUM-LS004'){
            res  = ' order by A.payroll_agen_reg_line_num,A.payroll_agen_org_num asc'
        }

        if(whereList[0].tableName == 'A_PAYROLL_AGEN_CUST_HOLD_PROD_DTL-LS003'){
            res  = ' order by A.cust_belong_super_org_num,A.cust_belong_org_num,A.cust_belong_org_num,A.cust_id asc'
        }
            ...
            if ((val != null && val != undefined && val != '')||item.nameEn=='SUPER_ORG_NUM'||item.nameEn=='ORG_NUM') {
                if (item.nameEn.toUpperCase() == 'CURR_CD') {//币种特殊处理                	
                	...
                	if(val=='001'){//统计业绩关联口径
                		...
                		res += " and A.ROLE_CD in ('002','999') "
                	}
                }else if(item.nameEn.toUpperCase() == 'NEW_CORP_SIZE_CD'){
                    if(val == 'all'){//四部委划型GS027
                    }else{
                        res += " and A."+item.nameEn+"='"+val+"'";
                    }
                }else if(item.nameEn.toUpperCase() == 'FOUR_TRANF'){
                    if(val == 'all'){//四部委划型GS026
                    }else{
                        res += " and A.new_corp_size_cd='"+val+"'";
                    }
                }else if(item.nameEn.toUpperCase() == 'BIZ_BREED_NUM'){
                    if(val == 'all'){//业务类型 SC027
                    }else{
                    	res += " and A."+item.nameEn+"='"+val+"'";
                    }
                }else if(item.nameEn.toUpperCase() == 'BIZ_BREED_CD'){
                    if(val == 'all'){//业务类型 SC025
                    }else{
                    	res += " and A."+item.nameEn+"='"+val+"'";
                    }
                }else {
                        ...
                        if (item.formType == '3') {
                        	...
                        	if(orgMap.hasOwnProperty(item.nameEn.toUpperCase())
                            	...
                            	if('A_COB_IN_OFF_BAL_BIZ_SITU_TAB-GS015'==whereList[0].tableName){
                            		if($("#ORG_HIRCHY").val()=='5'){
                            			res += " and A." + "SHOW_ORG_NUM" + " in (" + dealOrgWhere(val, item.nameEn) + ")";
                            		}else{
                            			res += " and (A.ORG_NUM in (" + dealOrgWhere(val, item.nameEn) + ") or (A.ORG_NUM in ('999999') and A.SHOW_ORG_NUM in (" + dealOrgWhere(val, item.nameEn) + ")))";
                            		}
                            	}else{
                                    if (gedai999999Arr.indexOf(whereList[0].tableName)>-1
                                        &&($("#SUPER_ORG_HIRCHY").val()=='2'
