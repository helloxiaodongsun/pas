...
$(function(){
    ...
    $.ajax({
           ...
           data: {
               'tableName': 'R_TRD_BANK_MDL_BIZ_ADDT-GS018_BL'
           },
           cache: false,
	...
	$("#btn_save").click(function(){
		...
		var index;
  		$.ajax({
  			url:portal.bp() + '/table/addRecord/GS018_BLsaveAll',  			
  			type:'post',
  			cache:false,
...
var TableObjPage = {
		table1: function () {
	              ...
	              [
	               {
	               ...
	               },
					{
					 ...
					 {
						 field:'stdpComnFeeIncomTwoAccm',
						 title:'结构性存款手续费收入双计（元）',
						 align: "center",
						 valign: "middle",
						 editable:{
							 type:'text',
							 ...
							 validate: function (v) {
			                        ...
			                        if (!checkFloat(v)) return '结构性存款手续费收入双计必须是数字';
			                        if(parseFloat(v)>99999999999999999.999999){
			                        	return '结构性存款手续费收入双计值过大';
			                        }
			                    }
						 }
					 },	
	        ...
	        $('#table1').bootstrapTable('destroy').bootstrapTable({
	            url: portal.bp() + '/table/addRecord/GS018_BLquery',
	            method: 'get',      //请求方式（*）
...
var TableObjPageHistory = {
		table2: function () {
		    ...
		    $('#table2').bootstrapTable('destroy').bootstrapTable({
		        url: portal.bp() + '/table/addRecord/GS018_BLqueryAll',
		        method: 'get',      //请求方式（*）
		        striped: true,      //是否显示行间隔色
		        cache: false,      //是否使用缓存，默认为true，所以一般情况下需要设置一下这个属性（*）
		        pagination: true,     //是否显示分页（*）
