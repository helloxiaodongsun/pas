...
//需要单独加cust_id字段的表
var custIdList = ['A_REB_CUST_CONTR_RK_TAB-ZH015'];
...
$(function () {
    ...
    createHtml();
    createJs();
    if(whereList[0].tableName=="A_COB_CUST_MGR_PER_RK_TAB-ZH012"||whereList[0].tableName=="A_REB_CUST_MGR_PER_RK_TAB-ZH013"){
    ...
    if(whereList[0].tableName=='A_COB_TEM_PER_RK_TAB-ZH014'){
   ...
   //初始化报表说明(备注)
    ...
    var tjkjs = $("#TJKJ").val();
    var tableNames = whereList[0].tableName;
    if (tjkjs != undefined && tjkjs != null && tjkjs != '' && tjkjs != 'undefined') {
    	tableNames += '-' + tjkjs;
    }
    $("#ORDER_TARGET").html("").append(
			$.param.getSelectOptionOrderTarget(tableNames))
			.selectpicker('refresh').selectpicker('val', "");
    $("#TJKJ").change(function(){
    	var tjkjls = $(this).val();
    	tableNames = whereList[0].tableName+'-'+tjkjls;
    	$("#ORDER_TARGET").html("").append(
    			$.param.getSelectOptionOrderTarget(tableNames))
    			.selectpicker('refresh').selectpicker('val', "");
...
var TableObj = {
    oTableInit: function () {
        ...
        $('#queryTable').bootstrapTable({
            url: portal.bp() + '/table/ZH011/query',
            method: 'post',      //请求方式（*）
          ...
          //  height:getTableHeight(document), //表格固定高度
            ...
            onLoadSuccess: function (data) {          
            	...
            	$("#queryTable").bootstrapTable('mergeCells',{index:length-1,field:"emp_id",colspan:4,rowspan:1});*/
                resizeTables();
            },
            columns: columns,
            rowStyle:rowStyle
...
var TableObjNotPage = {
    oTableInit: function () {
        ...
        var columns = $.param.getTableHeaderByType(tableType);
        $('#queryTable').bootstrapTable({
            url: portal.bp() + '/table/ZH011/queryNoPage',
            method: 'post',      //请求方式（*）
            striped: true,      //是否显示行间隔色
            cache: false,      //是否使用缓存，默认为true，所以一般情况下需要设置一下这个属性（*）
...
function createQueryParams(params, columns) {
    ...
    var isDisplayPntBal = "";
    var isCntnNp = "";
    var retailBizAssPrft = "";
    var wcurrDpstBal ="";
    var custLevel ="";
    var corpBizAssPrft ="";
    var wcurrCustBal =""
    var orderRule = $("#ORDER_RULE").selectpicker('val');
    var orderTarget = $("#ORDER_TARGET").selectpicker('val');
   // var orderRule1 = $("#ORDER_RULE").val();
   // var orderTarget1 = $("#ORDER_TARGET").val();
    queryParams.selectStr = function () {
        var res = '1';
        if(custIdList.indexOf(whereList[0].tableName)>-1){
        	res+=',A.cust_id'
        }
        $.each(whereList, function (index, item) {
        ...
        $.each(whereList, function (index, item) {
            ...
            if(item.nameEn.toUpperCase()=='RETAIL_BIZ_ASS_PRFT'){
            	retailBizAssPrft = $("#" + item.nameEn).val();
            }
            
            if(item.nameEn.toUpperCase()=='WCURR_DPST_DAILY_DPST_BAL'){
            	wcurrDpstBal = $("#" + item.nameEn).val();
            }
            if(item.nameEn.toUpperCase()=='CORP_BIZ_ASS_PRFT'){
            	corpBizAssPrft = $("#" + item.nameEn).val();
            }
            
            if(item.nameEn.toUpperCase()=='WCURR_CUST_DAILY_DPST_BAL'){
            	wcurrCustBal = $("#" + item.nameEn).val();
            }
            ...
            $.each(cols, function (index, item) {
                ...
                if (item.field != null && item.field != undefined) {
                	if(item.matrixingTag != undefined && item.matrixingTag != null){
                		...
                		if(item.field=='cust_y_daily_dpst_incremt_index_cmplt_ratio'){//ZH011
                			res +=", sum(case when V.INDEX_NUM='KOT0006' then v.NEGU_INDEX_CMPLT_ratio else 0 end) as "+item.field;
                		}else if(item.field=='cny_corp_y_daily_dpst_incremt_index_cmplt_ratio'){
                			res +=", sum(case when V.INDEX_NUM='KCP0003' then v.NEGU_INDEX_CMPLT_ratio else 0 end) as "+item.field;
                		}else if(item.field=='fcurr_corp_y_daily_dpst_incremt_index_cmplt_ratio'){
                			res +=", sum(case when V.INDEX_NUM='KCP0004' then v.NEGU_INDEX_CMPLT_ratio else 0 end) as "+item.field;
                		}else if(item.field=='dpst_y_daily_incremt_index_cmplt_ratio'){
                			res +=", sum(case when V.INDEX_NUM='KRE0002' then v.NEGU_INDEX_CMPLT_ratio else 0 end) as "+item.field;
                		}else if(item.field=='inc_fin_loan_cly_incremt_cmplt_ratio'){
                			res +=", sum(case when V.INDEX_NUM='KOT0026' then v.NEGU_INDEX_CMPLT_ratio else 0 end) as "+item.field;
                		}else if(item.field=='corp_inc_fin_loan_cly_incremt_cmplt_ratio'){
                			res +=", sum(case when V.INDEX_NUM='KCP0233' then v.NEGU_INDEX_CMPLT_ratio else 0  end) as "+item.field;
                		}else if(item.field=='retail_inc_fin_loan_cly_incremt_cmplt_ratio'){
                			res +=", sum(case when V.INDEX_NUM='KRE0135' then v.NEGU_INDEX_CMPLT_ratio else 0 end) as "+item.field;
                		}else if(item.field=='mdl_biz_incom_cmplt_ratio'){
                			res +=", sum(case when V.INDEX_NUM='KMI0030' then v.NEGU_INDEX_CMPLT_ratio else 0 end) as "+item.field;
                		}else if(item.field=='corp_mdl_biz_incom_cmplt_ratio'){
                			res +=", sum(case when V.INDEX_NUM='KMI0021' then v.NEGU_INDEX_CMPLT_ratio else 0 end) as "+item.field;
                		}else if(item.field=='inv_line_mdl_biz_incom_cmplt_ratio'){
                			res +=", sum(case when V.INDEX_NUM='KMI0043' then v.NEGU_INDEX_CMPLT_ratio else 0 end) as "+item.field;
                		}else if(item.field=='trd_bank_mdl_biz_incom_cmplt_ratio'){
                			res +=", sum(case when V.INDEX_NUM='KMI0042' then v.NEGU_INDEX_CMPLT_ratio else 0  end) as "+item.field;
                		}else if(item.field=='trust_mdl_biz_incom_cmplt_ratio'){
                			res +=", sum(case when V.INDEX_NUM='KMI0041' then v.NEGU_INDEX_CMPLT_ratio else 0  end) as "+item.field;
                		}else if(item.field=='retail_mdl_biz_incom_cmplt_ratio'){
                			res +=", sum(case when V.INDEX_NUM='KMI0022' then v.NEGU_INDEX_CMPLT_ratio else 0 end) as "+item.field;
                		}else if(item.field=='ass_prft_index_cmplt_ratio'){
                			res +=", sum(case when V.INDEX_NUM='KMI0040' then v.NEGU_INDEX_CMPLT_ratio else 0 end) as "+item.field;
                		}else if(item.field=='corp_biz_ass_prft_index_cmplt_ratio'){
                			res +=", sum(case when V.INDEX_NUM='KMI0044' then v.NEGU_INDEX_CMPLT_ratio else 0 end) as "+item.field;
                		}else if(item.field=='retail_biz_ass_prft_index_cmplt_ratio'){
                			res +=", sum(case when V.INDEX_NUM='KMI0045' then v.NEGU_INDEX_CMPLT_ratio else 0 end) as "+item.field;
                		}else if(item.field=='eva_cmplt_ratio'){
                			res +=", sum(case when V.INDEX_NUM='KOT0028' then v.NEGU_INDEX_CMPLT_ratio else 0 end) as "+item.field;
                		}else if(item.field=='whole_biz_score'){
                			res += ", round(sum(v.score)#2) as "+item.field;
                		}else if(item.field=='corp_biz_score'){
                			res += ", round(sum(case when v.BELONG_Line_cd='01' then v.score end)#2) as "+item.field;
                		}else if(item.field=='retail_biz_score'){
                			res += ", round(sum(case when v.BELONG_Line_cd='02' then v.score end)#2) as "+item.field;
                		}else if(item.field=='y_daily_dpst_incremt_index_cmplt_ratio'){//ZH012
                			res +=", sum(case when V.INDEX_NUM='KRE0001' then v.NEGU_INDEX_CMPLT_ratio else 0 end) as "+item.field;
                		}else if(item.field=='rmb_y_daily_dpst_incremt_index_cmplt_ratio'){
                			res +=", sum(case when V.INDEX_NUM='KCP0003' then v.NEGU_INDEX_CMPLT_ratio else 0 end) as "+item.field;
                		}else if(item.field=='fcurr_y_daily_dpst_incremt_index_cmplt_ratio'){
                			res +=", sum(case when V.INDEX_NUM='KCP0004' then v.NEGU_INDEX_CMPLT_ratio else 0 end) as "+item.field;
                		}else if(item.field=='micro_loan_pnt_cly_incremt_index_cmplt_ratio'){
                			res +=", sum(case when V.INDEX_NUM='KCP0098' then v.NEGU_INDEX_CMPLT_ratio else 0 end) as "+item.field;
                		}else if(item.field=='inc_fin_loan_cly_incremt_cmplt_ratio'){
                			res +=", sum(case when V.INDEX_NUM='KCP0233' then v.NEGU_INDEX_CMPLT_ratio else 0 end) as "+item.field;
                		}else if(item.field=='new_add_cust_cmplt_ratio'){
                			res +=", sum(case when V.INDEX_NUM='KCP0109' then v.NEGU_INDEX_CMPLT_ratio else 0  end) as "+item.field;
                		}else if(item.field=='inter_stl_qtty_index_cmplt_ratio'){
                			res +=", sum(case when V.INDEX_NUM='KMK0095' then v.NEGU_INDEX_CMPLT_ratio else 0 end) as "+item.field;
                		}else if(item.field=='corp_mdl_biz_incom_cmplt_ratio'){
                			res +=", sum(case when V.INDEX_NUM='KMI0021' then v.NEGU_INDEX_CMPLT_ratio else 0 end) as "+item.field;
                		}else if(item.field=='corp_biz_ass_prft_index_cmplt_ratio'){
                			res +=", sum(case when V.INDEX_NUM='LMI0044' then v.NEGU_INDEX_CMPLT_ratio else 0 end) as "+item.field;
                		}else if(item.field=='eva_cmplt_ratio'){
                			res +=", sum(case when V.INDEX_NUM='KCP0241' then v.NEGU_INDEX_CMPLT_ratio else 0 end) as "+item.field;
                		}else if(item.field=='dpst_y_daily_incremt_index_cmplt_ratio'){//ZH013
                			res +=", sum(case when V.INDEX_NUM='KRE0002' then v.NEGU_INDEX_CMPLT_ratio else 0 end) as "+item.field;
                		}else if(item.field=='capt_qtty_incremt_cmplt_ratio'){
                			res +=", sum(case when V.INDEX_NUM='KRE0001' then v.NEGU_INDEX_CMPLT_ratio else 0 end) as "+item.field;
                		}else if(item.field=='loan_cmplt_ratio'){
                			res +=", sum(case when V.INDEX_NUM='KRE0003' then v.NEGU_INDEX_CMPLT_ratio else 0 end) as "+item.field;
                		}else if(item.field=='inc_fin_loan_cmplt_ratio'){
                			res +=", sum(case when V.INDEX_NUM='KRE0133' then v.NEGU_INDEX_CMPLT_ratio else 0 end) as "+item.field;
                		}else if(item.field=='new_add_cust_cmplt_ratio'){
                			res +=", sum(case when V.INDEX_NUM='KRE0145' then v.NEGU_INDEX_CMPLT_ratio else 0 end) as "+item.field;
                		}else if(item.field=='new_add_vip_cust_cnt_cmplt_ratio'){
                			res +=", sum(case when V.INDEX_NUM='KRE0146' then v.NEGU_INDEX_CMPLT_ratio else 0 end) as "+item.field;
                		}else if(item.field=='retail_mdl_biz_incom_cmplt_ratio'){
                			res +=", sum(case when V.INDEX_NUM='KMI0022' then v.NEGU_INDEX_CMPLT_ratio else 0 end) as "+item.field;
                		}else if(item.field=='retail_biz_ass_prft_index_cmplt_ratio'){
                			res +=", sum(case when V.INDEX_NUM='KMI0045' then v.NEGU_INDEX_CMPLT_ratio else 0 end) as "+item.field;
                		}else if(item.field=='eva_cmplt_ratio'){
                			res +=", sum(case when V.INDEX_NUM='KRE0175' then v.NEGU_INDEX_CMPLT_ratio else 0 end) as "+item.field;
                		}else if(item.field=='y_daily_dpst_incremt_index_cmplt_ratio'){//ZH014
                			res +=", sum(case when V.INDEX_NUM='KRE0001' then v.NEGU_INDEX_CMPLT_ratio else 0 end) as "+item.field;
                		}else if(item.field=='rmb_y_daily_dpst_incremt_index_cmplt_ratio'){
                			res +=", sum(case when V.INDEX_NUM='KCP0003' then v.NEGU_INDEX_CMPLT_ratio else 0 end) as "+item.field;
                		}else if(item.field=='fcurr_y_daily_dpst_incremt_index_cmplt_ratio'){
    ...
    queryParams.tableStr = function () {
    	var val = $("#DATA_DT").val();
    	var res = whereList[0].tableName.split('-')[0];
    	if(val!=""&&val!=null){
	    	if(res=="A_ORG_PER_RK_TAB"){//ZH011
	    		res = whereList[0].dataBaseUser+'.'+whereList[0].tableName.split('-')[0]+
                    ...
                    " and v.index_num in('KOT0006','KCP0003','KCP0004','KRE0002','KOT0026','KCP0233','KRE0135'," +
                    "'KMI0030','KMI0021','KMI0043','KMI0042','KMI0041','KMI0022','KMI0040','KMI0044','KMI0045','KOT0028')" +
                    " and v.data_dt ='"+val+"' and v.ass_y = substr('"+val+"',1,4) and v.mon = substr('"+val+"',1,7) ";
				/*groupBy.push("V.INDEX_NUM","v.negu_index_cmplt_ratio",
						"case when V.INDEX_NUM = 'KOT0006' then v.NEGU_INDEX_CMPLT_ratio end",
            			"case when V.INDEX_NUM = 'KCP0003' then v.NEGU_INDEX_CMPLT_ratio end",
            			"case when V.INDEX_NUM = 'KCP0004' then v.NEGU_INDEX_CMPLT_ratio end",
            			"case when V.INDEX_NUM = 'KRE0002' then v.NEGU_INDEX_CMPLT_ratio end",
            			"case when V.INDEX_NUM = 'KOT0026' then v.NEGU_INDEX_CMPLT_ratio end",
            			"case when V.INDEX_NUM = 'KCP0233' then v.NEGU_INDEX_CMPLT_ratio end",
            			"case when V.INDEX_NUM = 'KRE0135' then v.NEGU_INDEX_CMPLT_ratio end",
            			"case when V.INDEX_NUM = 'KMI0030' then v.NEGU_INDEX_CMPLT_ratio end",
            			"case when V.INDEX_NUM = 'KMI0021' then v.NEGU_INDEX_CMPLT_ratio end",
            			"case when V.INDEX_NUM = 'KMI0043' then v.NEGU_INDEX_CMPLT_ratio end",
            			"case when V.INDEX_NUM = 'KMI0042' then v.NEGU_INDEX_CMPLT_ratio end",
            			"case when V.INDEX_NUM = 'KMI0041' then v.NEGU_INDEX_CMPLT_ratio end",
            			"case when V.INDEX_NUM = 'KMI0022' then v.NEGU_INDEX_CMPLT_ratio end",
            			"case when V.INDEX_NUM = 'KMI0040' then v.NEGU_INDEX_CMPLT_ratio end",
            			"case when V.INDEX_NUM = 'KMI0044' then v.NEGU_INDEX_CMPLT_ratio end",
            			"case when V.INDEX_NUM = 'KMI0045' then v.NEGU_INDEX_CMPLT_ratio end",
            			"case when V.INDEX_NUM = 'KOT0028' then v.NEGU_INDEX_CMPLT_ratio end");*/

	    	}else if(res=="A_COB_CUST_MGR_PER_RK_TAB"){//ZH012
	    		res = whereList[0].dataBaseUser+'.'+whereList[0].tableName.split('-')[0]+
	    				" A left join "+whereList[0].dataBaseUser+".A_EMP_CONVT_RATIO_V V on A.emp_id = v.emp_id " +
	    						" and v.index_num in('KRE0001','KCP0003','KCP0004','KCP0098','KCP0233','KCP0109','KMK0095','KMI0021','LMI0044','KCP0241') " +
	    						" and v.data_dt ='"+val+"' and v.ass_y = substr('"+val+"',1,4)  and v.mon = substr('"+val+"',1,7) ";
				/*groupBy.push("V.INDEX_NUM","v.negu_index_cmplt_ratio",
						"case when V.INDEX_NUM = 'KRE0001' then v.NEGU_INDEX_CMPLT_ratio end",
            			"case when V.INDEX_NUM = 'KCP0003' then v.NEGU_INDEX_CMPLT_ratio end",
            			"case when V.INDEX_NUM = 'KCP0004' then v.NEGU_INDEX_CMPLT_ratio end",
            			"case when V.INDEX_NUM = 'KCP0098' then v.NEGU_INDEX_CMPLT_ratio end",
            			"case when V.INDEX_NUM = 'KCP0233' then v.NEGU_INDEX_CMPLT_ratio end",
            			"case when V.INDEX_NUM = 'KCP0109' then v.NEGU_INDEX_CMPLT_ratio end",
            			"case when V.INDEX_NUM = 'KMK0095' then v.NEGU_INDEX_CMPLT_ratio end",
            			"case when V.INDEX_NUM = 'KMI0021' then v.NEGU_INDEX_CMPLT_ratio end",
            			"case when V.INDEX_NUM = 'LMI0044' then v.NEGU_INDEX_CMPLT_ratio end",
            			"case when V.INDEX_NUM = 'KCP0241' then v.NEGU_INDEX_CMPLT_ratio end");*/

	    	}else if(res=="A_REB_CUST_MGR_PER_RK_TAB"){//ZH013
	    		res = whereList[0].dataBaseUser+'.'+whereList[0].tableName.split('-')[0]+
	    				" A left join "+whereList[0].dataBaseUser+".A_EMP_CONVT_RATIO_V V on A.emp_id = v.emp_id " +
	    						" and v.index_num in('KRE0002','KRE0001','KRE0003','KRE0133','KRE0145','KRE0146','KMI0022','KMI0045','KRE0175') " +
	    						" and v.data_dt ='"+val+"' and v.ass_y = substr('"+val+"',1,4) and v.mon = substr('"+val+"',1,7) ";
				/*groupBy.push("V.INDEX_NUM","v.negu_index_cmplt_ratio",
						"case when V.INDEX_NUM = 'KRE0002' then v.NEGU_INDEX_CMPLT_ratio end",
            			"case when V.INDEX_NUM = 'KRE0001' then v.NEGU_INDEX_CMPLT_ratio end",
            			"case when V.INDEX_NUM = 'KRE0003' then v.NEGU_INDEX_CMPLT_ratio end",
            			"case when V.INDEX_NUM = 'KRE0133' then v.NEGU_INDEX_CMPLT_ratio end",
            			"case when V.INDEX_NUM = 'KRE0145' then v.NEGU_INDEX_CMPLT_ratio end",
            			"case when V.INDEX_NUM = 'KRE0146' then v.NEGU_INDEX_CMPLT_ratio end",
            			"case when V.INDEX_NUM = 'KMI0022' then v.NEGU_INDEX_CMPLT_ratio end",
            			"case when V.INDEX_NUM = 'KMI0045' then v.NEGU_INDEX_CMPLT_ratio end",
            			"case when V.INDEX_NUM = 'KRE0175' then v.NEGU_INDEX_CMPLT_ratio end");*/

	    	}else if(res=="A_COB_TEM_PER_RK_TAB"){//ZH014
	    		res = whereList[0].dataBaseUser+'.'+whereList[0].tableName.split('-')[0]+
	    				" A left join "+whereList[0].dataBaseUser+".A_TEM_CONVT_RATIO_V V on A.team_id = v.team_id " +
	    						" and v.index_num in('KRE0001','KCP0003','KCP0004','KCP0098','KCP0233','KCP0109','KMK0095','KMI0021','KMI0044','KCP0241') " +
	    						" and v.data_dt ='"+val+"' and v.ass_y = substr('"+val+"',1,4) and v.mon = substr('"+val+"',1,7) ";
				/*groupBy.push("V.INDEX_NUM","v.negu_index_cmplt_ratio",
						"case when V.INDEX_NUM = 'KRE0001' then v.NEGU_INDEX_CMPLT_ratio end",
            			"case when V.INDEX_NUM = 'KCP0003' then v.NEGU_INDEX_CMPLT_ratio end",
    ...
    //排序
    queryParams.orderStr1 = function () {
        ...
        if(orderOrgNum.indexOf(whereList[0].tableName)>-1){       	      
	        res = '  A.lev4_org_num,A.lev5_org_num '
	        res=resPrefix +res;
        }else{
        	if (flag) {
	            res = '';
  ...
  //排序
    queryParams.orderStr2 = function () {
       ...
       /* var flag = true;
                ...
                if(ORG_SEARCH != null && ORG_SEARCH != undefined){
                    ...
                    if (item.nameEn.toUpperCase() == 'ORG_NUM'
                        ...
                        org_num_field = org_num_field == null || org_num_field == undefined ?
                            $.param.getSelectOrgNumOrgNameRef('0' + org_hirchy, 'ORG_HIRCHY_ORG_NAME_REF') : org_num_field;
                        if(groupBy.indexOf(org_num_field.name.toLowerCase())<0){
                            groupBy.push("B."+org_num_field.name);
                        }
                        orderFieldIndex.push(item.isOrderBy);
                        var orderTmp = new Array();
                        orderTmp.push(org_num_field.name);
        ...
        if(orderOrgNum.indexOf(whereList[0].tableName)>-1){       	      
	        res = '  B.lev4_org_num,B.lev5_org_num '
	        res=resPrefix +res;
        }else{
        	if (flag) {
	            res = '';
	            ...
	            for(var i=0;i<orderArray.length;i++){
	                ...
	                if(i==0){
	                    ...
	                    res += "B."+orderFieldElement[0]+' '+ orderFieldElement[1];
	                }else{
	                    res += ',B.'+ orderFieldElement[0]+' '+ orderFieldElement[1];
	                }
	            }
        ...
        if(orderTarget!=''&&orderTarget!=null&&orderTarget!=undefined){
        	if(res!=''){
        		res += ',B.'+orderTarget+' '+orderRule 
        	}else{
        		res += ' B.'+orderTarget+' '+orderRule
        		res=resPrefix +res;
        	}      	
    ...
    queryParams.whereStr = function () {
        ...
        $.each(whereList, function (index, item) {
            ...
            if ((val != null && val != undefined && val != '')||item.nameEn=='SUPER_ORG_NUM'||item.nameEn=='ORG_NUM') {
                if (item.nameEn.toUpperCase() == 'CURR_CD') {//币种特殊处理
                	...
                	if(val=='001'){//统计业绩关联口径
                		res += " and A.CAPT_QTTY_PER_STAT_CALI in ('001','999') "
                	}else if(val=='002'){
                		res += " and A.CAPT_QTTY_PER_STAT_CALI in ('002','999') "
                	}
                }else if(item.nameEn.toUpperCase() == 'CUST_LEVEL'){
                	if(val=="0"){//客户等级
                	}else{
                        res += " and A."+item.nameEn+"='"+val+"'";
                	}
        ...
        //零售考核利润规模	
        if(retailBizAssPrft=="0"){
        }else if(retailBizAssPrft=="1"){
        	res+=" and A.RETAIL_BIZ_ASS_PRFT >= 300000 "
        }else if(retailBizAssPrft=="2"){
        	res+=" and A.RETAIL_BIZ_ASS_PRFT >= 100000 "
        }else if(retailBizAssPrft=="3"){
        	res+=" and A.RETAIL_BIZ_ASS_PRFT >= 50000 "
        }else if(retailBizAssPrft=="4"){
        	res+=" and A.RETAIL_BIZ_ASS_PRFT < 50000 "
        }	
        
        //储蓄日均余额
        if(wcurrDpstBal=="0"){
        }else if(wcurrDpstBal=="1"){
        	res+=" and A.WCURR_DPST_DAILY_DPST_BAL >= 6000000 "
        }else if(wcurrDpstBal=="2"){
        	res+=" and A.WCURR_DPST_DAILY_DPST_BAL >= 2000000 "
        }else if(wcurrDpstBal=="3"){
        	res+=" and A.WCURR_DPST_DAILY_DPST_BAL >= 500000 "
        }else if(wcurrDpstBal=="4"){
        	res+=" and A.WCURR_DPST_DAILY_DPST_BAL >= 200000 "
        }else if(wcurrDpstBal=="5"){
        	res+=" and A.WCURR_DPST_DAILY_DPST_BAL >= 50000 "
        }else if(wcurrDpstBal=="6"){
        	res+=" and A.WCURR_DPST_DAILY_DPST_BAL < 50000 "
        }
        //公司考核利润
        if(corpBizAssPrft=="0"){	
        }else if(corpBizAssPrft=="1"){
        	res+=" and A.CORP_BIZ_ASS_PRFT >= 10000000 "
        }else if(corpBizAssPrft=="2"){
        	res+=" and A.CORP_BIZ_ASS_PRFT >= 5000000 "
        }else if(corpBizAssPrft=="3"){
        	res+=" and A.CORP_BIZ_ASS_PRFT >= 1000000 "
        }else if(corpBizAssPrft=="4"){
        	res+=" and A.CORP_BIZ_ASS_PRFT >= 500000 "
        }else if(corpBizAssPrft=="5"){
        	res+=" and A.CORP_BIZ_ASS_PRFT >= 50000 "
        }else if(corpBizAssPrft=="6"){
        	res+=" and A.CORP_BIZ_ASS_PRFT < 50000 "
        }
        //日均存款余额
        if(wcurrCustBal=="0"){	
        }else if(wcurrCustBal=="1"){
        	res+=" and A.WCURR_CUST_DAILY_DPST_BAL >= 1000000000 "
        }else if(wcurrCustBal=="2"){
        	res+=" and A.WCURR_CUST_DAILY_DPST_BAL >= 500000000 "
        }else if(wcurrCustBal=="3"){
        	res+=" and A.WCURR_CUST_DAILY_DPST_BAL >= 100000000 "
        }else if(wcurrCustBal=="4"){
        	res+=" and A.WCURR_CUST_DAILY_DPST_BAL >= 50000000 "
        }else if(wcurrCustBal=="5"){
        	res+=" and A.WCURR_CUST_DAILY_DPST_BAL >= 10000000 "
        }else if(wcurrCustBal=="6"){
        	res+=" and A.WCURR_CUST_DAILY_DPST_BAL >= 5000000 "
        }else if(wcurrCustBal=="7"){
        	res+=" and A.WCURR_CUST_DAILY_DPST_BAL < 5000000 "
        }
...
function exportCurrentPageExcel(){
	...
	publicReportExcel(lastQueryParams,"exportExcelZH011");
}
