...
var gedai999999Arr = ['A_REB_LOAN_SIZE_SITU_TAB-LS034','A_REB_LOAN_SIZE_SITU_TAB-LS035','A_REB_LOAN_SIZE_SITU_TAB-LS036'
                      ...
                      ,'A_COB_MICRO_LOAN_NUM_STAT_TAB-GS023'];
//排序管辖行-支行
var orderOrgNum = ['A_PLF_OPRR_PURE_CORP_MDL_BIZ_INCOM-JC011','A_PLF_OPRR_ALL_CORP_MDL_BIZ_INCOM-JC012','A_PLF_OPRR_MDL_BIZ_TWO_ACCM_INCOM-JC013'
                   ,'A_REB_TIME_MATURE-LS010','A_REB_STDP_MATURE-LS011','A_REB_NATION_DEBT_MATURE-LS012'];
//默认分页最多50行
...
$(function () {
    ...
    createHtml();
    createJs();
    if(whereList[0].tableName=="A_REB_TIME_MATURE-LS010"||whereList[0].tableName=="A_REB_STDP_MATURE-LS011"
    	||whereList[0].tableName=="A_REB_NATION_DEBT_MATURE-LS012"){
...
function createQueryParams(params, columns) {
    ...
    queryParams.selectStr = function () {
        ...
        $.each(whereList, function (index, item) {
            ...
            if(item.nameEn.toUpperCase()=='CORP_TYPE'){
            	corpType = $("#" + item.nameEn).val();
            }
                ...
                if (item.field != null && item.field != undefined) {
                	if(item.matrixingTag != undefined && item.matrixingTag != null){
                		if(item.field !='y_daily_index'&& item.field !='year_incremt_index'&& item.field !='mon_incremt_index'
                			&& item.field !='mon_target'&& item.field !='mon_index_diff'&& item.field !='negu_index_val_1'
                				&& item.field !='negu_index_val_2'){
                			if(whereList[0].tableName=='A_COB_CUST_PROD_DTL-GS013'){
                				res += ', sum(A.' + item.field+') as '+item.field;
                			}else if(item.field == 'crnt_bal'||item.field == 'crnt_bal_cny'){
                				if(whereList[0].tableName=='A_REB_FIN_MATURE_DTL_TAB_MGR-LS008'){
                					res += ', sum(A.' + item.field+') as '+item.field;
                				}else{
	                			     res += ", sum(case when A.ROLE_CD IN ('001'#'999')  then A." + item.field+" else 0 end) as "+item.field;
                				}
                			}else{
                				if(whereList[0].tableName=='A_COB_MICRO_LOAN_NUM_STAT_TAB-GS023'){
                					if (currCdVal == '01') {//人民币
                			...
                			if(item.field =='year_incremt_index' && whereList[0].tableName=='A_REB_CAPT_REP_REG_LINE-LS016'){//LS016
                    			if(currCdVal == '01'){//人民币
                    				res +=", (case when V.INDEX_NUM='KRE0052' then v.Y_NEGU_INDEX_VAL end) as "+item.field;
                    			}else if(currCdVal == '02'){//外折人
                    				res +=", (case when V.INDEX_NUM='KRE0053' then v.Y_NEGU_INDEX_VAL end) as "+item.field;
                    			}else{//本外币
                    				res +=", (case when V.INDEX_NUM='KRE0001' then v.Y_NEGU_INDEX_VAL end) as "+item.field;
                    			}
                    		}else if(item.field =='mon_incremt_index' && whereList[0].tableName=='A_REB_CAPT_REP_REG_LINE-LS016'){//LS016
                    			if(currCdVal == '01'){//人民币
                    				res +=", (case when V.INDEX_NUM='KRE0052' then v.NEGU_INDEX_VAL end) as "+item.field;
                    			}else if(currCdVal == '02'){//外折人
                    				res +=", (case when V.INDEX_NUM='KRE0053' then v.NEGU_INDEX_VAL end) as "+item.field;
                    			}else{//本外币
                    				res +=", (case when V.INDEX_NUM='KRE0001' then v.NEGU_INDEX_VAL end) as "+item.field;
                    			}
                    		}else if(item.field =='mon_index_diff' && whereList[0].tableName=='A_REB_CAPT_REP_REG_LINE-LS016'){//LS016
                    			if(currCdVal == '01'){//人民币
                    				res +=", (case when V.INDEX_NUM='KRE0052' then v.NEGU_INDEX_VAL end)-sum(A.MB_INCREMT_IN_TRANS) as "+item.field;
                    			}else if(currCdVal == '02'){//外折人
                    				res +=", (case when V.INDEX_NUM='KRE0053' then v.NEGU_INDEX_VAL end)-sum(A.MB_INCREMT_IN_TRANS) as "+item.field;
                    			}else{//本外币
                    				res +=", (case when V.INDEX_NUM='KRE0001' then v.NEGU_INDEX_VAL end)-sum(A.MB_INCREMT_IN_TRANS) as "+item.field;
                    			}
                    		}else if(item.field =='year_incremt_index' && whereList[0].tableName=='A_REB_DPST_REP_REG_LINE-LS018'){//LS018
                    			if(currCdVal == '01'){//人民币
                    				res +=", (case when V.INDEX_NUM='KRE0057' then v.Y_NEGU_INDEX_VAL end) as "+item.field;
                    			}else if(currCdVal == '02'){//外折人
                    				res +=", (case when V.INDEX_NUM='KRE0058' then v.Y_NEGU_INDEX_VAL end) as "+item.field;
                    			}else{//本外币
                    				res +=", (case when V.INDEX_NUM='KRE0002' then v.Y_NEGU_INDEX_VAL end) as "+item.field;
                    			}
                    		}else if(item.field =='mon_incremt_index' && whereList[0].tableName=='A_REB_DPST_REP_REG_LINE-LS018'){//LS018
                    			if(currCdVal == '01'){//人民币
                    				res +=", (case when V.INDEX_NUM='KRE0057' then v.NEGU_INDEX_VAL end) as "+item.field;
                    			}else if(currCdVal == '02'){//外折人
                    				res +=", (case when V.INDEX_NUM='KRE0058' then v.NEGU_INDEX_VAL end) as "+item.field;
                    			}else{//本外币
                    				res +=", (case when V.INDEX_NUM='KRE0002' then v.NEGU_INDEX_VAL end) as "+item.field;
                    			}
                    		}else if(item.field =='mon_index_diff' && whereList[0].tableName=='A_REB_DPST_REP_REG_LINE-LS018'){//LS018
                    			if(currCdVal == '01'){//人民币
                    				res +=", (case when V.INDEX_NUM='KRE0057' then v.NEGU_INDEX_VAL end)-sum(A.MB_INCREMT_IN_TRANS) as "+item.field;
                    			}else if(currCdVal == '02'){//外折人
                    				res +=", (case when V.INDEX_NUM='KRE0058' then v.NEGU_INDEX_VAL end)-sum(A.MB_INCREMT_IN_TRANS) as "+item.field;
                    			}else{//本外币
                    				res +=", (case when V.INDEX_NUM='KRE0002' then v.NEGU_INDEX_VAL end)-sum(A.MB_INCREMT_IN_TRANS) as "+item.field;
                    			}
                    		}else if(item.field =='mon_target' && whereList[0].tableName=='A_REB_LOAN_SIZE_SITU_TAB-LS034'){//LS034
                    			if(currCdVal == '01'){//人民币
                    				res +=", (case when V.INDEX_NUM='KRE0068' then v.NEGU_INDEX_VAL end) as "+item.field;
                    			}else if(currCdVal == '02'){//外折人
                    				res +=", (case when V.INDEX_NUM='KRE0069' then v.NEGU_INDEX_VAL end) as "+item.field;
                    			}else{//本外币
                    				res +=", (case when V.INDEX_NUM='KRE0003' then v.NEGU_INDEX_VAL end) as "+item.field;
                    			}
                    		}else if(item.field =='negu_index_val_1' && whereList[0].tableName=='A_COB_MICRO_LOAN_NUM_STAT_TAB-GS023'){//GS023
                				if(corpType=="0"){
                    				res +=", (case when V.INDEX_NUM='KCP0243' then v.NEGU_INDEX_VAL end) as "+item.field;
                				}else if(corpType=="1"){
                    				res +=", (case when V.INDEX_NUM='KCP0247' then v.NEGU_INDEX_VAL end) as "+item.field;
                				}
	                		}else if(item.field =='negu_index_val_2' && whereList[0].tableName=='A_COB_MICRO_LOAN_NUM_STAT_TAB-GS023'){//GS023
	                    			if(corpType=="0"){
	                    				res +=", (case when V.INDEX_NUM='KCP0244' then v.NEGU_INDEX_VAL end) as "+item.field;
	                				}else if(corpType=="1"){
	                    				res +=", (case when V.INDEX_NUM='KCP0248' then v.NEGU_INDEX_VAL end) as "+item.field;
	                				}
	                		}else{
	                			res += ',A.' + item.field;
                	...
                	}else if(item.field == 'relr_info'){     
                		if(whereList[0].tableName=="A_REB_DIR_CONN_POS_RECV_STAT-LS032"){
            				res += ",string_agg(A."+item.field+"#'#') AS "+item.field;
                		}else{
                			res += ",string_agg(CASE WHEN A.EMP_NUM LIKE '%HGH' THEN NULL ELSE A.RELR_INFO END#'#') AS "+item.field;
                		}
                	}else{
                		if(item.field =='m_index_cmplt_ratio' && whereList[0].tableName=='A_REB_CAPT_REP_REG_LINE-LS016'){//LS016
                			if(currCdVal == '01'){//人民币
                				res +=", (case when V.INDEX_NUM='KRE0052' then v.NEGU_INDEX_CMPLT_ratio end) as "+item.field;
                			}else if(currCdVal == '02'){//外折人
                				res +=", (case when V.INDEX_NUM='KRE0053' then v.NEGU_INDEX_CMPLT_ratio end) as "+item.field;
                			}else{//本外币
                				res +=", (case when V.INDEX_NUM='KRE0001' then v.NEGU_INDEX_CMPLT_ratio end) as "+item.field;
                			}
                		}else if(item.field =='m_index_cmplt_ratio' && whereList[0].tableName=='A_REB_DPST_REP_REG_LINE-LS018'){//LS018
                			if(currCdVal == '01'){//人民币
                				res +=", (case when V.INDEX_NUM='KRE0057' then v.NEGU_INDEX_CMPLT_ratio end) as "+item.field;
                			}else if(currCdVal == '02'){//外折人
                				res +=", (case when V.INDEX_NUM='KRE0058' then v.NEGU_INDEX_CMPLT_ratio end) as "+item.field;
                			}else{//本外币
                				res +=", (case when V.INDEX_NUM='KRE0002' then v.NEGU_INDEX_CMPLT_ratio end) as "+item.field;
                			}
                		}else if(item.field =='target_rate' && whereList[0].tableName=='A_REB_LOAN_SIZE_SITU_TAB-LS034'){//LS034
                			if(currCdVal == '01'){//人民币
                				res +=", (case when V.INDEX_NUM='KRE0068' then v.NEGU_INDEX_CMPLT_ratio end) as "+item.field;
                			}else if(currCdVal == '02'){//外折人
                				res +=", (case when V.INDEX_NUM='KRE0069' then v.NEGU_INDEX_CMPLT_ratio end) as "+item.field;
                			}else{//本外币
                				res +=", (case when V.INDEX_NUM='KRE0003' then v.NEGU_INDEX_CMPLT_ratio end) as "+item.field;
                			}
                		}else if(item.field =='negu_index_cmplt_ratio_1' && whereList[0].tableName=='A_COB_MICRO_LOAN_NUM_STAT_TAB-GS023'){//GS023
            				if(corpType=="0"){
                				res +=", (case when V.INDEX_NUM='KCP0243' then v.negu_index_cmplt_ratio end) as "+item.field;
            				}else if(corpType=="1"){
                				res +=", (case when V.INDEX_NUM='KCP0247' then v.negu_index_cmplt_ratio end) as "+item.field;
            				}
	            		}else if(item.field =='negu_index_cmplt_ratio_2' && whereList[0].tableName=='A_COB_MICRO_LOAN_NUM_STAT_TAB-GS023'){//GS023
	                			if(corpType=="0"){
	                				res +=", (case when V.INDEX_NUM='KCP0244' then v.negu_index_cmplt_ratio end) as "+item.field;
	            				}else if(corpType=="1"){
	                				res +=", (case when V.INDEX_NUM='KCP0248' then v.negu_index_cmplt_ratio end) as "+item.field;
	            				}
	            		}else{
	            			if(item.field =="yb_num"||item.field =="qb_num"||item.field =="mb_num"||item.field =="lb_num"
	            				||item.field =="d_num"||item.field =="yb_num_incremt"||item.field =="qb_num_incremt"||
	            				item.field =="mb_num_incremt"||item.field =="cld_num_incremt"||item.field =="m_dd_num"){//户数sum但不根据单位换算
           					 	res += ', sum(A.' + item.field+') as '+item.field;
	            			}else{
	            				//字段脱敏
    ...
    queryParams.tableStr = function () {
       // return whereList[0].dataBaseUser+'.'+whereList[0].tableName.split('-')[0];
    	...
    	if(val!=""&&val!=null){
	    	if(res=="A_REB_CAPT_REP_REG_LINE-LS016"){//LS016
	    		res = whereList[0].dataBaseUser+'.'+whereList[0].tableName.split('-')[0]+
	    				" A left join "+whereList[0].dataBaseUser+".A_ORG_CONVT_RATIO_V V on A.org_num = v.org_num and v.org_hirchy in('2','4') and v.data_dt ='"+val+"' and v.ass_y = substr('"+val+"',1,4) ";
	    		...
	    		res = whereList[0].dataBaseUser+'.'+whereList[0].tableName.split('-')[0]+
	    				" A left join "+whereList[0].dataBaseUser+".A_ORG_CONVT_RATIO_V V on A.org_id = v.org_id and v.data_dt ='"+val+"' and v.ass_y = substr('"+val+"',1,4) ";
				groupBy.push('V.INDEX_NUM','v.NEGU_INDEX_VAL','v.NEGU_INDEX_CMPLT_ratio','v.data_dt','v.ass_y');
	
	    	}else if(res=="A_COB_MICRO_LOAN_NUM_STAT_TAB-GS023"){//GS023
	    		res = whereList[0].dataBaseUser+'.'+whereList[0].tableName.split('-')[0]+
				" A left join "+whereList[0].dataBaseUser+".A_ORG_CONVT_RATIO_V V on A.org_id = v.org_id and v.index_num in('KCP0244','KCP0243','KCP0247','KCP0248') " +
						" and v.data_dt ='"+val+"' and v.ass_y = substr('"+val+"',1,4) and v.mon = substr('"+val+"',1,7)";
				groupBy.push('V.INDEX_NUM','v.NEGU_INDEX_VAL','NEGU_INDEX_CMPLT_ratio','v.data_dt','v.ass_y');
    		...
    		if(res=="A_REB_CAPT_REP_REG_LINE-LS016"){//LS016
	    		...
	    		res = whereList[0].dataBaseUser+'.'+whereList[0].tableName.split('-')[0]+
				" A left join "+whereList[0].dataBaseUser+".A_ORG_CONVT_RATIO_V V on A.org_id = v.org_id ";
				groupBy.push('V.INDEX_NUM','v.NEGU_INDEX_VAL','NEGU_INDEX_CMPLT_ratio');
    ...
    queryParams.whereStr = function () {
        ...
        $.each(whereList, function (index, item) {
            ...
            if ((val != null && val != undefined && val != '')||item.nameEn=='SUPER_ORG_NUM'||item.nameEn=='ORG_NUM') {
                ...
                }else if(item.nameEn.toUpperCase() == 'CORP_TYPE'){//企业类型
                	if (val == '0') {
                        res += " and A.CORP_TYPE='文化金融'";
                    } else if (val == '1') {
                        res += " and A.CORP_TYPE='科技金融'";
                    }
                }else if(item.nameEn.toUpperCase() == 'RELA_ROLE'){
                	if(val=='001'){//统计业绩关联口径
                		res += " and (A.RELA_ROLE ='001' OR A.RELA_ROLE IS NULL) "
                	}else if(val=='002'){
                		res += " and (A.RELA_ROLE ='001' OR A.RELA_ROLE IS NULL) "
                	}
