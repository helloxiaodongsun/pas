...
$(function(){
	...
	//保存新增
		$("#btn_save1").click(function(){
			...
			var index;
	  		$.ajax({
	  			url:portal.bp() + '/table/addRecord/ZHBL_004SaveAll',
	  			type:'post',
	  			cache:false,
...
var TableObjPage = {
		table1: function(){
	        ...
	        $('#table1').bootstrapTable('destroy').bootstrapTable({
	            url: portal.bp() + '/table/addRecord/zhBl004QueryRows',
	            method: 'get',      //请求方式（*）
		...
		table2: function () {
	        ...
	        $('#table2').bootstrapTable('destroy').bootstrapTable({
	            url: portal.bp() + '/table/addRecord/ZHBL_004Query',
	            method: 'get',      //请求方式（*）
	            striped: true,      //是否显示行间隔色
	            cache: false,      //是否使用缓存，默认为true，所以一般情况下需要设置一下这个属性（*）
	           // pagination: true,     //是否显示分页（*）
...
var TableObjPageHistory = {
		table3: function () {
		    ...
		    $('#table3').bootstrapTable('destroy').bootstrapTable({
		        url: portal.bp() + '/table/addRecord/ZHBL_004QueryAll',
		        method: 'get',      //请求方式（*）
		        striped: true,      //是否显示行间隔色
		        cache: false,      //是否使用缓存，默认为true，所以一般情况下需要设置一下这个属性（*）
		        pagination: true,     //是否显示分页（*）
...
function vaildIsAdd(mon) {
	var num ="";
	$.ajax({
		url: portal.bp() + '/table/ZH_BL_004/validIsAdd?r='+Math.random(),
		type: 'get',
		async: false,
		data: {
			"mon": mon
