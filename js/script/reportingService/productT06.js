...
$(function () {
   ...
   /* $("#queryTable").on("click","#showModel",function(){
...
function query() {
    ...
    $("#queryTable").find("thead").find("tr:eq(0)").find("th:eq(3)").find("div:eq(0)").empty();
    $("#queryTable").find("thead").find("tr:eq(0)").find("th:eq(3)").find("div:eq(0)").prepend('<a href="javascript:void(0)" ' +
        ' id="showModel" onclick="clicks(this)"  subTableName="A_COB_DPSIT_LOAN_RATIO_CUST-GS011_1">存贷款比明细</a>');
}

function clicks(){	
        //var tableName = $(this).attr("subTableName");
        var tableName = 'A_COB_DPSIT_LOAN_RATIO_CUST-GS011_1';     
        var subTableWhere;
        var subWhereValue = {};
        $.ajax({
            url: portal.bp() + '/table/getTableWhere',
