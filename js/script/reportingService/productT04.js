...
var TableObjNoPage = {
    oTableInit: function () {
        ...
        var columns = $.param.getTableHeaderByType(tableType);
        $('#queryTable').bootstrapTable({
            url: portal.bp() + '/table/T04/queryNoPage',
            method: 'post',      //请求方式（*）
            striped: true,      //是否显示行间隔色
            cache: false,      //是否使用缓存，默认为true，所以一般情况下需要设置一下这个属性（*）
...
window.operateEvents = {
    "click #showModel": function (e, value, row, index) {
        ...
        var list = $("#queryTable").bootstrapTable('getData')[index];
        var rowName =list["class_desc"];
        ...
        if (queryIsPageFinder(tableName)) {
            TableObjSub.oTableInit(tableName, subWhereValue, subTableWhere,rowName,colName);
        } else {
            TableObjSubNoPage.oTableInit(tableName, subWhereValue, subTableWhere,rowName,colName);
        }
...
//下钻表分页查询列表
var TableObjSub = {
    oTableInit: function (tableName, subWhereValue, subTableWhere, rowName, colName) {
        ...
        $('#detailModelTable').bootstrapTable({
            ...
            contentType: "application/x-www-form-urlencoded",
            queryParams: function (params) {
                return createSubQueryParams(params, columns, tableName, subWhereValue, subTableWhere,rowName, colName);
            },
            sidePagination: "server",   //分页方式：client客户端分页，server服务端分页（*）
            pageNum: 1,      //初始化加载第一页，默认第一页
            pageSize: 100,      //每页的记录行数（*）
...
function createQueryParams(params, columns) {
            ...
            $.each(cols, function (index, item) {
                if (item.field != null && item.field != undefined) {
                    if(item.matrixingTag != undefined && item.matrixingTag != null){
                    	...
                    	if(whereList[0].tableName=='A_COB_CUST_DPST_MOVE_ORG-GS005'){
                    		if (currCdVal == '01') {//人民币
   	               			 res += ", sum(case when CURR_CD='01'  then " + item.field+" else 0 end) as "+item.field;
   	                        } else if (currCdVal == '02') {//外折人
   	                       	 res += ", sum(case when CURR_CD='02'  then " + item.field+" else 0 end) as "+item.field;
   	                        } else {//本外币
      	                       	 res += ", sum(case when CURR_CD='00'  then " + item.field+" else 0 end) as "+item.field;
   	                        }
                		}else{
	                		if (currCdVal == '01') {//人民币
	               			 res += ", sum(case when CURR_CD='01'  then " + item.field+" else 0 end) as "+item.field;
    ...
    queryParams.orderStr = function () {
        var resPrefix ='order by CLASS_CD asc';
        var res = '';
        var flag = true;
                        ...
                        if (item.formType == '3') {
                        	...
                        	if(whereList[0].tableName=="A_COB_CUST_DPST_MOVE_ORG-GS005"){
	                        	if(orgMap.hasOwnProperty(item.nameEn.toUpperCase())
	                        		...
	                        		if(orgHirchy == "5"){
	                        			if(val!=null&&val!=""){
			                        		res += " and seq_num = '1' and ORG_NUM='" + val + "'";
	                        			}else{
			                        		res += " and seq_num = '1' and ORG_NUM='0'";
	                        			}
	                        		}else{
	                        			if(val!=null&&val!=""){
			                        		res += " and seq_num = '2' and ORG_NUM='" + val + "'";
	                        			}else{
			                        		res += " and seq_num = '2' and ORG_NUM='0'";
	                        			}
	                        		}
	                        	}else{
...
function createSubQueryParams(params, columns, tableName, subWhereValue, subTableWhere,rowName, colName) {
    var subqueryParams = {
        ...
        $.each(columns, function (i, cols) {
            $.each(cols, function (index, item) {
                ...
                if (item.field != null && item.field != undefined) {
                    if(item.matrixingTag != undefined && item.matrixingTag != null){
                		//res += ', sum(' + item.field+') AS '+item.field;
                    	if(whereList[0].tableName=='A_COB_CUST_DPST_CHANGE_CUST-GS005_1'){
                    		if (currCdVal == '01') {//人民币
   	               			 res += ", sum(case when CURR_CD='01'  then " + item.field+" else 0 end) as "+item.field;
   	                        } else if (currCdVal == '02') {//外折人
   	                       	 res += ", sum(case when CURR_CD='02'  then " + item.field+" else 0 end) as "+item.field;
    ...
    subqueryParams.whereStr = function () {
        ...
        var tjkj = $("#TJKJ").val();
        //rowName, colName
        var indexNum = colName .lastIndexOf("_");
        var colCode = colName.substring(indexNum+1,colName.length);
        var rowCode = $.param.getSelectCustomsClassify(rowName, 'CUST_CLASSIFY_CODE').encode;
        var orgNum = $("#SUPER_ORG_NUM").val();
        var orgHirchy = $("#SUPER_ORG_HIRCHY").val();    
        if(tjkj=="DAY"){
            whereStr += ' and SUM_CRNT_BAL_FLG ='+rowCode+' and SUM_LAST_DT_BAL_FLG='+colCode;
        }else if(tjkj=="YEAR"){
            whereStr += ' and SUM_Y_DAILY_BAL_FLG ='+rowCode+' and SUM_LY_Y_DAILY_BAL_FLG='+colCode;
        }else if(tjkj=="MONTH"){
            whereStr += ' and SUM_M_DAILY_BAL_FLG ='+rowCode+' and SUM_LM_M_DAILY_BAL_FLG='+colCode;
        }else{
            whereStr += ' and SUM_CRNT_BAL_FLG ='+rowCode+' and SUM_LAST_DT_BAL_FLG='+colCode;
        }
        if(orgHirchy =='5'){
        	whereStr += ' and lev5_org_num ='+orgNum;
        }else if(orgHirchy =='4'){
        	whereStr += ' and lev4_org_num ='+orgNum;
        }else if(orgHirchy =='3'){
        	whereStr += ' and lev3_org_num ='+orgNum;
        }else if(orgHirchy =='2'){
        	whereStr += ' and lev2_org_num ='+orgNum;
        }else if(orgHirchy =='1'){
        	whereStr += ' and lev1_org_num ='+orgNum;
      ...
      //去重
        ...
        whereStr +=',lev4_org_num,lev5_org_num  order by lev4_org_num,lev5_org_num '
