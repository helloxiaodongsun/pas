...
$(function(){
    ...
    $.ajax({
           ...
           data: {
               'tableName': 'R_RETAIL_DPST_QTY_TRAN_OUT_INFO-LS017_1'
           },
           cache: false,
	...
	$("#btn_save").click(function(){
		...
		var index;
  		$.ajax({
  			url:portal.bp() + '/table/addRecord/LS017_1saveAll',  			
  			type:'post',
  			cache:false,
...
var TableObjPage = {
		table1: function () {
			                    ...
			                    validate: function (v) {
			                    	...
			                    	if (v==null||v==''){
			                    		...
			                    		if(branchName==null||branchName==''){
			                    			enableSiblings($el,"tranInBrchName");
			                    			//enableSiblingsAndSetValue($el,"tranInOrgName","");
			                    		}else{
			                    			disableSiblingsAndSetValue($el,"tranInBrchName",branchName);
			                    		}
			                    	}
			                    }
			                }
	        ...
	        $('#table1').bootstrapTable('destroy').bootstrapTable({
	            url: portal.bp() + '/table/addRecord/LS017_1query',
	            method: 'get',      //请求方式（*）
...
var TableObjPageHistory = {
		table2: function () {
		    ...
		    $('#table2').bootstrapTable('destroy').bootstrapTable({
		        url: portal.bp() + '/table/addRecord/LS017_1queryAll',
		        method: 'get',      //请求方式（*）
		        striped: true,      //是否显示行间隔色
		        cache: false,      //是否使用缓存，默认为true，所以一般情况下需要设置一下这个属性（*）
		        pagination: true,     //是否显示分页（*）
