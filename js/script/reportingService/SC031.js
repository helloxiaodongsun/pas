...
$(function(){
    ...
    $.ajax({
           ...
           data: {
               'tableName': 'R_M_AGN_HQ_ALLOC_ADDT-SC031'
           },
           cache: false,
	...
	$("#btn_save").click(function(){
		...
		var index;
  		$.ajax({
  			url:portal.bp() + '/table/addRecord/SC031saveAll',  			
  			type:'post',
  			cache:false,
...
var TableObjPage = {
		table1: function () {
					 ...
					 },
					 {
						 ...
						 editable:{
							 ...
							 validate: function (v) {
			                        ...
			                        if (!checkFloat(v)) return '当月代销总行划拨必须是数字';
			                    }
						 }
					 },
					 {
	        ...
	        $('#table1').bootstrapTable('destroy').bootstrapTable({
	            url: portal.bp() + '/table/addRecord/SC031query',
	            method: 'get',      //请求方式（*）
...
var TableObjPageHistory = {
		table2: function () {
		    ...
		    $('#table2').bootstrapTable('destroy').bootstrapTable({
		        url: portal.bp() + '/table/addRecord/SC031queryAll',
		        method: 'get',      //请求方式（*）
		        striped: true,      //是否显示行间隔色
		        cache: false,      //是否使用缓存，默认为true，所以一般情况下需要设置一下这个属性（*）
		        pagination: true,     //是否显示分页（*）
