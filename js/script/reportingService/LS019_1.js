...
$(function(){
	 $("#url").attr("href",portal.bp()+"/table/T01/index/A_REB_DPST_REP_SUB_BRCH-LS019?mid=3350");
	//初始化报表说明(备注)
    $.ajax({
           ...
           data: {
               'tableName': 'R_RETAIL_DPST_QTY_TRAN_OUT_INFO-LS019_1'
           },
           cache: false,
	...
	$("#btn_save").click(function(){
		...
		var index;
  		$.ajax({
  			url:portal.bp() + '/table/addRecord/LS019_1saveAll',  			
  			type:'post',
  			cache:false,
...
var TableObjPage = {
		table1: function () {
			                    ...
			                    validate: function (v) {
			                    	...
			                    	if (v==null||v==''){
			                    		...
			                    		if(branchName==null||branchName==''){
			                    			enableSiblings($el,"tranInIrchName");
			                    			//enableSiblingsAndSetValue($el,"tranInOrgName","");
			                    		}else{
			                    			disableSiblingsAndSetValue($el,"tranInIrchName",branchName);
			                    		}
			                    	}
			                    }
			                }
							 ...
							 validate: function (v) {
								 $el = $(this);
			                    	if (v==null||v==''){
			                    		...
			                    		var branchName = $.param.getBranchName(v,'0');
			                    		if(branchName==null||branchName==''){
			                    			enableSiblings($el,"tranOutBrchName");
			                    			//enableSiblingsAndSetValue($el,"tranInOrgName","");
			                    		}else{
			                    			disableSiblingsAndSetValue($el,"tranOutBrchName",branchName);
			                    		}
			                    	}
			                    }
						 }
					 ...
					 },
					 {
						 ...
						 editable:{
							 ...
							 validate: function (v) {
			                    	//if (v==null||v=='') return '必输项';
								 if(v!=null&&v!=''&&v!='空'){
			                        if (!checkFloat(v)) return '累计日均转移金额必须是数字';
								 }
			                 }
						 }
					 },
					 {
						 field:'mDailyTranOutAmt',
						 title:'月日均转移金额（监管指标）',
						 align: "center",
						 valign: "middle",
						 editable:{
							 ...
							 validate: function (v) {
			                    	//if (v==null||v=='') return '必输项';
								 if(v!=null&&v!=''&&v!='空'){
			                        if (!checkFloat(v)) return '月日均转移金额必须是数字';
								 }
			                 }
						 }
					 },
	        ...
	        $('#table1').bootstrapTable('destroy').bootstrapTable({
	            url: portal.bp() + '/table/addRecord/LS019_1query',
	            method: 'get',      //请求方式（*）
...
var TableObjPageHistory = {
		table2: function () {
		    ...
		    $('#table2').bootstrapTable('destroy').bootstrapTable({
		        url: portal.bp() + '/table/addRecord/LS019_1queryAll',
		        method: 'get',      //请求方式（*）
		        striped: true,      //是否显示行间隔色
		        cache: false,      //是否使用缓存，默认为true，所以一般情况下需要设置一下这个属性（*）
		        pagination: true,     //是否显示分页（*）
