...
$(function () {
    ...
    if(whereList[0].tableName=="A_REB_CUST_OPEN_CARD_INFO-LS027"){
    	$("#ACCT_CATE").html("").append(
    			$.param.getAcctCate())
    			.selectpicker('refresh').selectpicker('val', "0");
    	$("#CARD_NAME").html("").append(
    			$.param.getCardName())
    			.selectpicker('refresh').selectpicker('val', "0");
    }
    query();
...
*/                     		res += ', sum(A.' + item.field+') as '+item.field;
                		...
                		if(whereList[0].tableName=="A_REB_CUST_OPEN_CARD_INFO-LS027"){
                			if(item.field=="camp_person_id"||item.field=="camp_person_name"||item.field=="camp_person_ratio"){
    ...
    queryParams.whereStr = function () {
        ...
        $.each(whereList, function (index, item) {
            ...
            if ((val != null && val != undefined && val != '')||item.nameEn=='SUPER_ORG_NUM'||item.nameEn=='ORG_NUM') {
                ...
                }else if(item.nameEn.toUpperCase() == 'ACCT_CATE'){
                	if(val=='0'){//账户类别
                	}else{
                		res += " and A.ACCT_CATE = '"+ val+"'";
                	}
                }else if(item.nameEn.toUpperCase() == 'CARD_NAME'){
                	if(val=='0'){//卡名称
                	}else{
                		res += " and A.CARD_NAME ='"+ val+"'";
                	}
                }else if(item.nameEn.toUpperCase() == 'STATUS'){
                	if(val=='0'){//状态
                		res += " and A.STATUS in ('开卡','换卡') "
                	}else if(val=='1'){
                		res += " and A.STATUS = '销卡' "
                	}
                }else {
