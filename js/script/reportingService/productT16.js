...
var gedai999999Arr = ['A_REB_LOAN_SIZE_SITU_TAB-LS034','A_REB_LOAN_SIZE_SITU_TAB-LS035','A_REB_LOAN_SIZE_SITU_TAB-LS036'
                      ...
                      ,'A_MKB_IBANK_DPST_QRY-SC002','A_MKB_GUAR_BIZ_QRY_ORG-SC004'];
...
function createQueryParams(params, columns) {
        ...
        $.each(columns, function (i, cols) {
            $.each(cols, function (index, item) {
                if (item.field != null && item.field != undefined) {
                    if(item.field.toUpperCase()  == 'CUST_DTL_LINK' || item.field.toUpperCase() == 'EMP_DTL_LINK'){
                        ...
                        if(item.matrixingTag != undefined && item.matrixingTag != null){
                            ...
                            if(item.field =='target_rate' && whereList[0].tableName=='A_COB_DPST_BAL_ORG_TAB-GS001'){//GS001
                                ...
                                res +=",  round(sum(A.loan_pnt_bal)/sum(A.dpsit_pnt_bal)*100#2)  as "+item.field;
    ...
    queryParams.whereStr = function () {
        ...
        if(whereList[0].tableName=="A_MKB_INTER_STL_QTTY_ORG-SC003"){//SC003
    		res += ",A.ORG_NUM,A.SEQ_NUM "
    	}
        whereStr = res;
        return res;
    }
...
function createSubQueryParams(params, columns, tableName, subWhereValue, subTableWhere) {
    ...
    subqueryParams.whereStr = function () {
        ...
        $.each(whereList, function (index, item) {
            ...
            if ((val != null && val != undefined && val != '')||item.nameEn=='SUPER_ORG_NUM'||item.nameEn=='ORG_NUM') {
                if (item.nameEn.toUpperCase() == 'CURR_CD') {//币种特殊处理
                	...
                	if(tableName=='A_MKB_IBANK_DPST_QRY_EMP_DTL-SC002_2'||tableName=='A_MKB_INTER_STL_QTTY_EMP-SC003_2'){
                		if(val=='001'){//统计业绩关联口径
                			whereStr += " and (ROLE_CD ='001' OR ROLE_CD IS NULL) "
                    	}else if(val=='002'){
                    		whereStr += " and (ROLE_CD ='002' OR ROLE_CD IS NULL) "
                    	}
                	}
      ...
      //去重
        ...
        /*if(tableName=="A_MKB_INTER_STL_QTTY_CUST-SC003_1"){//SC003_1
        	whereStr += ",LEV4_ORG_NUM,LEV5_ORG_NUM "
    	}*/
       /* if(tableName=="A_MKB_INTER_STL_QTTY_EMP-SC003_2"){//SC003_2
        	whereStr += ",CUST_MGR_EMPNO "
    	}*/
        return whereStr;
    }
    //排序
    subqueryParams.orderStr = function () {
        ...
        if(whereList[0].tableName == 'A_MKB_GUAR_BIZ_QRY_ORG-SC004'||whereList[0].tableName == 'A_MKB_IBANK_DPST_QRY-SC002'){
        ...
        if(tableName=="A_MKB_INTER_STL_QTTY_EMP-SC003_2"){//SC003_2
    		res = " order by LEV4_ORG_NUM,LEV5_ORG_NUM,CUST_MGR_EMPNO "
    	}
