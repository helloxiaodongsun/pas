...
//排序管辖行-支行
var orderOrgNum = ['A_PLF_OPRR_PURE_CORP_MDL_BIZ_INCOM-JC011','A_PLF_OPRR_ALL_CORP_MDL_BIZ_INCOM-JC012','A_PLF_OPRR_MDL_BIZ_TWO_ACCM_INCOM-JC013','A_REB_INDV_LOAN_WEIGHT_RATE-LS044'];
//排序显示机构SHOW_ORG_NUM
            ...
            $.each(cols, function (index, item) {
                ...
                if (item.field != null && item.field != undefined) {
                	if(item.matrixingTag != undefined && item.matrixingTag != null){
                	    ...
                	    if(whereList[0].tableName=='A_REB_INDV_LOAN_WEIGHT_RATE-LS044'){
                            if(item.field.indexOf("_rate")!=-1){
                                res+=",  round(sum("+item.field+")/sum(case when  "+item.field.split('_')[0]+"_y_"+item.field.split('_')[1]+"= 0 then null else  "+item.field.split('_')[0]+"_y_"+item.field.split('_')[1]+" end)#2) as "+item.field;
                            }else{
                                //字段脱敏
