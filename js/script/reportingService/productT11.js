...
$(function () {
    ...
    if(whereList[0].tableName == 'A_REB_HOLD_PROD_DTL_REP-LS056'){ //持有产品报表_明细表
        if(isCustMgr == '1') {
            $("#CUST_MGR_EMPNO").val(empNum);
            $("#CUST_MGR_EMPNO").attr('disabled', true);
        }
    }

    if(whereList[0].tableName == 'A_REB_HOLD_PROD_SUM_REP-LS062'){ //持有产品报表_汇总表
        if(isCustMgr == '1') {
            $("#EMP_ID").val(empNum);
...
var TableObjNotPage = {
    oTableInit: function () {
        ...
        var columns = $.param.getTableHeaderByType(tableType);
        $('#queryTable').bootstrapTable({
            url: portal.bp() + '/table/T11/queryNoPage',
            method: 'post',      //请求方式（*）
            striped: true,      //是否显示行间隔色
            cache: false,      //是否使用缓存，默认为true，所以一般情况下需要设置一下这个属性（*）
...
function createQueryParams(params, columns) {
    ...
    queryParams.selectStr = function () {
        ...
        $.each(whereList, function (index, item) {
            ...
            if(item.nameEn=='CUS_CLASS'){
                cusClass = $("#" + item.nameEn).val();
            }
            ...
            $.each(cols, function (index, item) {
                ...
                if (item.field != null && item.field != undefined) {
                	if(item.matrixingTag != undefined && item.matrixingTag != null){
        			    ...
        			    if(item.field=="day_daily_asset_90"){
                           //res += ',' + item.field;
        			    	//修改为需要sum的字段
                		...
                		if(whereList[0].tableName == 'A_REB_HOLD_PROD_SUM_REP-LS062'){
                			if(item.field=="cust_cnt"||item.field=="large_dpst"||item.field=="brk_evn_fin"||item.field=="non_brk_evn_fin"
                				||item.field=="thd_pty_agn"||item.field=="fund"||item.field=="insure"||item.field=="pre_metals"
                					||item.field=="credit_card"||item.field=="st"||item.field=="mobbank"||item.field=="indv_loan"){
                				res += ', sum(A.' + item.field + ') as ' + item.field;
                			}else{
                				//字段脱敏
    ...
    queryParams.tableStr = function () {
        return whereList[0].dataBaseUser+'.'+whereList[0].tableName.split('-')[0]+" A ";
    ...
    queryParams.orderStr = function () {
        ...
        if(whereList[0].tableName == 'A_REB_HOLD_PROD_SUM_REP-LS062'){
            res = ' order by A.super_org_num,A.belong_org_num asc';
        }
        return res;
    }
    ...
    queryParams.whereStr = function () {
        ...
        $.each(whereList, function (index, item) {
            ...
            if ((val != null && val != undefined && val != '')||item.nameEn=='SUPER_ORG_NUM'||item.nameEn=='ORG_NUM') {
                ...
                }else if(item.nameEn.toUpperCase() == 'CUST_MGR_EMPNO'){
                    if(val != undefined || val != '' || val!=null) {//客户经理号
                        res += " and A." + item.nameEn + " like ('%" + val + "%')";
                    }
                } else {
                    if (item.iswhere == '1') {
                        if (item.formType == '3') {
        ...
        // 资产达标标志 和 贵宾卡标志
        if(cusClass =='0'){
        	res+=" and A.VISTNT_CARD_FLG = '1' "
        }else if(cusClass=='1'){
            res+=" and A.ASSET_RCH_STD_FLG = '1' "
        }
