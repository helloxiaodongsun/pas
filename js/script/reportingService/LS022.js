...
var TableObj = {
    oTableInit: function () {
        ...
        $('#queryTable').bootstrapTable({
            url: portal.bp() + '/table/LS022/query',
            method: 'post',      //请求方式（*）
...
var TableObjNotPage = {
    oTableInit: function () {
        ...
        var columns = $.param.getTableHeaderByType(tableType);
        $('#queryTable').bootstrapTable({
            url: portal.bp() + '/table/LS022/queryNoPage',
            method: 'post',      //请求方式（*）
            striped: true,      //是否显示行间隔色
            cache: false,      //是否使用缓存，默认为true，所以一般情况下需要设置一下这个属性（*）
                	...
                	if(item.matrixingTag != undefined && item.matrixingTag != null){
                		if(whereList[0].tableName=="A_COB_MICRO_CUST_MGR_PER_TAB-GS027"){
                			...
                			if(item.field.toUpperCase() == 'STATR_FUNDS'){
                				...
                				res += ",sum(case when DATA_DT =date'"+date_e+"' then ASSET_BAL else 0 end) " +
                                "- sum(case when DATA_DT =date'"+date_s+"' then ASSET_BAL else 0 end) as "+item.field;
                			}else{
                				res += ', sum(' + item.field+') AS '+item.field;
                			}
                		}else{
            ...
            if ((val != null && val != undefined && val != '')) {
                if (item.iswhere == '1') {
                    if(item.formType =='2'&&item.nameEn.toUpperCase()=='ASSET_BAL'){
                    	...
                    	if(inputData!=""&&inputData!=undefined){
                    		inputData = inputData +"0000";
                    		res += " A.funds_diff >='" + inputData + "'";
                    	}else{
                    		val = val +"0000";
                            res += " A.funds_diff >='" + val + "'";
                    	}
                    }
                }
            }
...
function exportCurrentPageExcel(){
    ...
    publicReportExcel(lastQueryParams,"exportExcelLS022");
}
