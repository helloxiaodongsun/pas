...
function createQueryParams(params, columns) {
        ...
        $.each(columns, function (i, cols) {
            $.each(cols, function (index, item) {
                ...
                if (item.field != null && item.field != undefined) {
                	...
                	}else if(item.field =='cust_mgr_num'|| item.field =='cust_mgr_name'
            			|| item.field =='cust_mgr_belong_org_num'|| item.field =='cust_mgr_belong_org_name'){     
    ...
    queryParams.orderStr = function () {
        ...
        }else if(whereList[0].tableName =="A_REB_FUND_PER_REP_EMP-LS058" ){
        	res = ' order by A.lev4_org_num,A.lev5_org_num,A.FUND_PROD_CD,A.CUST_NUM,A.CUST_MGR_NUM '
        }else if(whereList[0].tableName =="A_REB_FUND_PER_REP_CUST-LS049" ){
        	res = ' order by A.lev4_org_num,A.lev5_org_num,A.CUST_NUM,A.AGT_ID,A.CUST_MGR_NUM '
        }else{
        	if (flag) {
	            res = '';
	        }else {
    ...
    queryParams.whereStr = function () {
        ...
        $.each(whereList, function (index, item) {
            ...
            if ((val != null && val != undefined && val != '')||item.nameEn=='SUPER_ORG_NUM'||item.nameEn=='ORG_NUM') {
                ...
                }else if(item.nameEn.toUpperCase() == 'FUND_PROD_NAME'){
                    //基金产品名称
