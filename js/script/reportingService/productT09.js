...
var gedai999999Arr = ['A_REB_LOAN_SIZE_SITU_TAB-LS034','A_REB_LOAN_SIZE_SITU_TAB-LS035','A_REB_LOAN_SIZE_SITU_TAB-LS036'
                      ...
                      ,'A_PLF_CUST_LOAN_TAB-JC005','A_PLF_CUST_LOAN_TAB-JC006','A_PLF_CUST_AMT_FUND_TAB-JC007','A_COB_IN_OFF_BAL_BIZ_SITU_TAB-GS015'
                      ];
//排序管辖行-支行
...
var TableObjNotPage = {
    oTableInit: function () {
        ...
        var columns = $.param.getTableHeaderByType(tableType);
        $('#queryTable').bootstrapTable({
            url: portal.bp() + '/table/T09/queryNoPage',
            method: 'post',      //请求方式（*）
            striped: true,      //是否显示行间隔色
            cache: false,      //是否使用缓存，默认为true，所以一般情况下需要设置一下这个属性（*）
...
function createQueryParams(params, columns) {
                	...
                	if(item.matrixingTag != undefined && item.matrixingTag != null){
                		if(item.field !='y_daily_index'&& item.field !='year_incremt_index'&& item.field !='mon_incremt_index'
                			&& item.field !='mon_target'&& item.field !='mon_index_diff'){
	                		 ...
	                		 if (currCdVal == '01') {//人民币
	                			 res += ", sum(case when A.CURR_CD IN ('01'#'00')  then A." + item.field+" else 0 end) as "+item.field;
	                         } else if (currCdVal == '02') {//外折人
	                        	 res += ", sum(case when A.CURR_CD IN ('02'#'00')  then A." + item.field+" else 0 end) as "+item.field;
	                         } else{//本外币
                		...
                		}else if("A_REB_HIRCHY_CUST_STRU-LS026"==whereList[0].tableName && item.field=="cust_cnt"){
                			if (currCdVal == '01') {//人民币
	                			 res += ", sum(case when A.CURR_CD IN ('01'#'00')  then A." + item.field+" else 0 end) as "+item.field;
	                         } else if (currCdVal == '02') {//外折人
	                        	 res += ", sum(case when A.CURR_CD IN ('02'#'00')  then A." + item.field+" else 0 end) as "+item.field;
    ...
    queryParams.orderStr = function () {
        ...
        if(whereList[0].tableName=="A_REB_HIRCHY_CUST_STRU-LS026"){
        	res = ' order by A.org_num asc,A.seq_num asc,A.CUST_CLASS_CD desc '
        }
    ...
    queryParams.whereStr = function () {
        ...
        $.each(whereList, function (index, item) {
            ...
            if ((val != null && val != undefined && val != '')||item.nameEn=='SUPER_ORG_NUM'||item.nameEn=='ORG_NUM') {
                if (item.nameEn.toUpperCase() == 'CURR_CD') {//币种特殊处理                	
                	/*if(whereList[0].tableName=='A_REB_HIRCHY_CUST_STRU-LS026'){
        				 if (val == '01') {//人民币
                             res += " and A.CURR_CD IN ('01','00')";
                         } else if (val == '02') {//外折人
                             res += " and A.CURR_CD IN ('02','00')";
                         } else if (val == '03') {//本外币
        ...
        if(whereList[0].tableName=="A_REB_HIRCHY_CUST_STRU-LS026"){
        	res += ',A.org_num,A.seq_num,A.CUST_CLASS_CD '
        }
