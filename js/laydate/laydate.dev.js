...
Dates.orien = function(obj, pos){
    var tops, rect = Dates.elem.getBoundingClientRect();
     //top: 46, right: 617.8125, bottom: 72, left: 453.8125, width: 164
    console.log(rect);
    //console.log(Dates.elem.offsetLeft);
    if(Dates.winarea(1)-rect.left>=obj.offsetWidth){
        obj.style.left = rect.left + (pos ? 0 : Dates.scroll(1)) + 'px';
    }else {
        var chazhi=(rect.left+obj.offsetWidth)-Dates.winarea(1);
        obj.style.left=rect.left+(pos ? 0 : Dates.scroll(1))-chazhi-(Dates.winarea(1)-rect.left-Dates.elem.offsetWidth)+'px';
    }
