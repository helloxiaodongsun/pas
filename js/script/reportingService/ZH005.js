...
$(function(){
    ...
    $("#CUST_NAME").change(function(){
        ...
        var orgCode =  $.param.getSelectOptionOrderOrgCode(custName);
    ...
    $("#ORG_CODE").change(function(){
        ...
        var custName =  $.param.getSelectOptionOrderCustName(orgCode);
    ...
    $("#tab8").click(function(){
    	TableObjNotPage.table8();
    });
    $("#tab9").click(function(){
    	TableObjNotPageTotal.table9();
    });
	//初始化报表说明(备注)
	$.ajax({
...
function exportCurrentPageExcel(){
	...
	if(tableId != "table8" && tableId!="table9"){
		//需要合并单元格的列，格式:  字段名1:列数1,字段名2:列数2
		lastQueryParams.mergeColumns="product_firsttitle:0";
	}
	publicReportExcel(lastQueryParams,"exportExcelZH005/"+tableId);
}
function exportAllDataExcel(){
	var reportExcelDTO_ZH005 = [];
	var d1 = {"tableNo":1};
	var d2 = {"tableNo":2};
	var d3 = {"tableNo":3};
	var d4 = {"tableNo":4};
	...
	reportExcelDTO_ZH005.push(d8);
	
	var columns9 = $("#table9").bootstrapTable('getOptions').columns;
	var result9 = tableNeedCopy(columns9,0,true);
	d9.tableHead = JSON.stringify(result9);
	d9.tableName = $("#tab9").text();
	reportExcelDTO_ZH005.push(d9);
	
	
	
	
	var data = {
		"reportExcelMoreTable":reportExcelDTO_ZH005,
		'dATA_DT':lastQueryParams.DATA_DT,
    	'cUST_NAME':lastQueryParams.CUST_NAME,
    	'oRG_CODE':lastQueryParams.ORG_CODE,
    	'cURR_CD':lastQueryParams.CURR_CD,
    	'dW':lastQueryParams.DW,
	}
	$.ajax({
		url : portal.bp() + '/report/exportExcelZH005',
		type:'post',
		cache:false,
		async : false,
...
function initTable(tableId,columns){
	$('#'+tableId).bootstrapTable('destroy').bootstrapTable({
        url: portal.bp() + '/table/zhTable/ZH005/'+tableId,
        method: 'get',      //请求方式（*）
	...
	$('#'+tableId).bootstrapTable('destroy').bootstrapTable({
        ...
        onLoadSuccess: function (data) {
        	...
        	$.each(data,function(index,item){
        		...
        		sum12 = parseFloat(sum12)+parseFloat(item.bill_discount_yb_incremt);
        		sum13 = parseFloat(sum13)+parseFloat(item.dir_inv_crnt_bal);
        		sum14 = parseFloat(sum14)+parseFloat(item.dir_inv_y_daily_bal);
        		sum15 = parseFloat(sum15)+parseFloat(item.dir_inv_yb_incremt);
        		...
        		sum18 = parseFloat(sum18)+parseFloat(item.ibank_asset_yb_incremt);
        		sum19 = parseFloat(sum19)+parseFloat(item.stru_fin_crnt_bal);
        		sum20 = parseFloat(sum20)+parseFloat(item.stru_fin_y_daily_bal);
        		sum21 = parseFloat(sum21)+parseFloat(item.stru_fin_yb_incremt);
        		sum22 = parseFloat(sum22)+parseFloat(item.send_crnt_bal);
        		sum23 = parseFloat(sum23)+parseFloat(item.send_y_daily_bal);
        		sum24 = parseFloat(sum24)+parseFloat(item.send_yb_incremt);
        		sum25 = parseFloat(sum25)+parseFloat(item.bank_acpt_crnt_bal);
        		sum26 = parseFloat(sum26)+parseFloat(item.bank_acpt_y_daily_bal);
        		sum27 = parseFloat(sum27)+parseFloat(item.bank_acpt_yb_incremt);
        		sum28 = parseFloat(sum28)+parseFloat(item.guar_crnt_bal);
        		sum29 = parseFloat(sum29)+parseFloat(item.guar_y_daily_bal);
        		sum30 = parseFloat(sum30)+parseFloat(item.guar_yb_incremt);
        		sum31 = parseFloat(sum31)+parseFloat(item.lc_crnt_bal);
        		sum32 = parseFloat(sum32)+parseFloat(item.lc_y_daily_bal);
        		sum33 = parseFloat(sum33)+parseFloat(item.lc_yb_incremt);
        	...
        	rows.push({
        		sub_cust_name:"合计",
	    		 ...
	    		 cust_dpst_yb_incremt:sum3.toFixed(2),
	    		 ibank_liab_crnt_bal:sum4.toFixed(2),
	    		 ibank_liab_y_daily_bal:sum5.toFixed(2),
	    		 ibank_liab_yb_incremt:sum6.toFixed(2),
	    		 cust_loan_crnt_bal:sum7.toFixed(2),
	    		 cust_loan_y_daily_bal:sum8.toFixed(2),
	    		 cust_loan_yb_incremt:sum9.toFixed(2),
	    		 bill_discount_crnt_bal:sum10.toFixed(2),
	    		 bill_discount_y_daily_bal:sum11.toFixed(2),
	    		 bill_discount_yb_incremt:sum12.toFixed(2),
	    		 dir_inv_crnt_bal:sum13.toFixed(2),
	    		 dir_inv_y_daily_bal:sum14.toFixed(2),
	    		 dir_inv_yb_incremt:sum15.toFixed(2),
	    		 ibank_asset_crnt_bal:sum16.toFixed(2),
	    		 ibank_asset_y_daily_bal:sum17.toFixed(2),
	    		 ibank_asset_yb_incremt:sum18.toFixed(2),
	    		 stru_fin_crnt_bal:sum19.toFixed(2),
	    		 stru_fin_y_daily_bal:sum20.toFixed(2),
	    		 stru_fin_yb_incremt:sum21.toFixed(2),
	    		 send_crnt_bal:sum22.toFixed(2),
	    		 send_y_daily_bal:sum23.toFixed(2),
	    		 send_yb_incremt:sum24.toFixed(2),
	    		 bank_acpt_crnt_bal:sum25.toFixed(2),
	    		 bank_acpt_y_daily_bal:sum26.toFixed(2),
	    		 bank_acpt_yb_incremt:sum27.toFixed(2),
	    		 guar_crnt_bal:sum28.toFixed(2),
	    		 guar_y_daily_bal:sum29.toFixed(2),
	    		 guar_yb_incremt:sum30.toFixed(2),
	    		 lc_crnt_bal:sum31.toFixed(2),
	    		 lc_y_daily_bal:sum32.toFixed(2),
	    		 lc_yb_incremt:sum33.toFixed(2)
        	});
        	$("#table9").bootstrapTable('append',rows);
//        	var length = $("#table9").bootstrapTable("getData").length;
//        	$("#table9").bootstrapTable('mergeCells',{index:length-1,field:"sub_cust_name",colspan:3,rowspan:1});
	    	               ...
	    	               [
	    	                ...
	    	                },
	    	                {
		    	                ...
		    	                [
			    	                ...
			    	                {
			    	                	field:'bill_discount_yb_incremt',
			    	                	title:'较年初',
			    	                	rowspan: 1,
			    	                	colspan: 1,
			    	                	align: "center",
